...
export default class OrderProductManager extends NavigationMixin(InputBase) {
	...
	getDeliveryGroupRecordTypeId(recordTypeName) {
		const recordTypeInfos = this.deliveryGroupObjectInfo.data?.recordTypeInfos;
		if (!recordTypeInfos)
			return null;

		const match = Object.values(recordTypeInfos)
			.find(rt => rt.name == recordTypeName || rt.developerName == recordTypeName);

		return match?.recordTypeId ||
			this.deliveryGroupObjectInfo.data?.defaultRecordTypeId ||
			null;
	}
