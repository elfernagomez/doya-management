...
import getPackingSlipsForOrder from '@salesforce/apex/PackingSlipManager.getPackingSlipsForOrder';
import getOrderProductsForOrder from '@salesforce/apex/PackingSlipManager.getOrderProductsForOrder';
...
/**
 * Creates an order product wrapper from Apex record
 * @param {Object} record - Apex OrderItem record
 * @returns {Object} Order product wrapper
 */
function createOrderProductWrapper(record) {
    return {
        Id: record.Id,
...
export default class OrderPackingSlipManager extends LightningElement {
    ...
    wiredOrderProducts({ error, data }) {
        if (data) {
            this.orderProducts = data.map(record => createOrderProductWrapper(record));
        } else if (error) {
